...
T_Menunew::~T_Menunew(void)
{
	DestroyAll();
}

void T_Menunew::SetMoveSound(AudioDXBuffer* ms_buffer)
{
	m_MoveSound = ms_buffer;
}

void T_Menunew::SetClickSound(AudioDXBuffer* mc_buffer)
{
	m_ClickSound = mc_buffer;
}

//设置菜单背景图片
void T_Menunew::SetMenuBkg(wstring img_path, int alphaLevel, COLORREF backColor)
{
...
//添加正常菜单图片
void T_Menunew::SetBtnBmp(wstring img_path, int btnWidth, int btnHeight)
{
	if(img_path.length()>0)
	{
...
//添加菜单文字项目
void T_Menunew::AddMenuItem(MENUITEM menuItem)
{
...
//设置首菜单项的坐标及菜单间隔距离
void T_Menunew::SetMenuInfo(MENU_INFO menuInfo)
{
	menu_info = menuInfo;
}

//菜单绘制
void T_Menunew::DrawMenu(HDC hdc, BYTE btnTrans, bool startState)
{	
...
//根据当前鼠标坐标计算菜单项索引号
int T_Menunew::GetMenuIndex(int x, int y)
{
	int Index = -1;
	POINT pt;
	pt.x = x;
...
StringAlignment T_Menunew::GetAlignment()
{
...
FontStyle T_Menunew::GetFontStyle()
{
	FontStyle fStyle;
...
//菜单鼠标点击事件处理
int T_Menunew::MenuMouseClick(int x, int y)
{
...
//菜单鼠标移动事件处理
void T_Menunew::MenuMouseMove(int x, int y)
{
...
int T_Menunew::MenuKeyDown(WPARAM key)
{
